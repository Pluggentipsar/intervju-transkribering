...
# CORS for local development, Tauri desktop app, and hosted frontend
# When frontend is hosted externally (Vercel, Netlify, etc.), it connects to localhost:8000
cors_origins = [
    "http://localhost:3000",
    "http://127.0.0.1:3000",
...
# Add hosted frontend origins from environment variable
# Set CORS_ORIGINS=https://tysttext.vercel.app,https://example.com
import os
extra_origins = os.getenv("CORS_ORIGINS", "")
if extra_origins:
    cors_origins.extend([o.strip() for o in extra_origins.split(",") if o.strip()])

# Also allow any HTTPS origin in hybrid mode (frontend hosted, backend local)
# This is safe since the backend only runs locally and requires user to start it
app.add_middleware(
    CORSMiddleware,
    allow_origins=cors_origins,
    allow_origin_regex=r"https://.*",  # Allow any HTTPS origin for hosted frontends
